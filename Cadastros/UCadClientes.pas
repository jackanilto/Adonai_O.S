...
uses
  ...
  frxClass, frxExportBaseDialog, frxExportPDF, Vcl.DBCtrls, Vcl.Menus;
...
procedure TFrmCadClientes.associarCampos;
  begin
     ...
     DM.TBL_CLIENTES.FieldByName('CNPJCPF')     .AsString := EditCNPJCPF.Text;
...
begin
  if (editNOME.Text <> '') then
    begin
        ...
        DM.QueryClientes.Close;
        DM.QueryClientes.SQL.Clear;
        DM.QueryClientes.SQL.Add('update TBL_CLIENTES set '+
       ' NOME              =  :NOME                 , '+
       ' ENDERECO          =  :ENDERECO             , '+
       ' NUMERO            =  :NUMERO               , '+
       ' BAIRRO            =  :BAIRRO               , '+
...
procedure TFrmCadClientes.Deletar1Click(Sender: TObject);
begin
if Messagedlg('Deseja excluir o registro?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
  begin
   associarCampos;
...
procedure TFrmCadClientes.Editar1Click(Sender: TObject);
begin
  if (editNOME.Text <> '') then
    begin
        associarCampos;
...
procedure TFrmCadClientes.Novo1Click(Sender: TObject);
begin
  //apos editar um dado esta desbilitando o Edit
  grid.Enabled:=false;
  EditNOME.Enabled := true; // Reabilita Edit
...
procedure TFrmCadClientes.Salva1Click(Sender: TObject);
begin
  if (EditNOME.Text <> '') then
  begin
    associarCampos;
